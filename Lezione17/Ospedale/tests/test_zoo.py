...
class TestZoo(TestCase):
    ...
    def test_animal_demension_after_feeding(self):
        ...
        result :bool = ((animal1.height* 1.02 )*(animal1.width * 1.02) > fence1.area)
        message :str = "Error. The animal1 should not be feeded"
        self.assertEqual(result,True,message)

    def test_feeding_animal(self):
        #Nome : Marim Arafa questo test controlla se i guardiani hanno dato da mangiare agli animali
        zoo :Zoo = Zoo()
        animal1 = Animal(name="Scoiattolo", species="Canide", height=10.0, width =1.0, age=10, preferred_habitat="Continent")
        fence1 = Fence(area=100, temperature=25, habitat="Continent")
        zoo_keeper1 = ZooKeeper(name="Lorenzo", surname="Maggi", id=1234)
        zoo_keeper1.feed(animal1,fence1)
        result :bool = ((animal1.height* 1.02 )*(animal1.width * 1.02) < fence1.area)
        message :str = "Error. The animal1 should be feeded"
        self.assertEqual(result,True,message)

    def test_cleaning_the_fence(self):
        pass
